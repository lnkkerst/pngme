...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        let (header, chunk_bytes): (&[u8], &[u8]) = value.split_at(8);
        ...
        let mut pos: usize = 0;
        loop {
            let length: u32 = u32::from_be_bytes(chunk_bytes[pos..pos + 4].try_into().unwrap());
            chunks.push(
                Chunk::try_from(&chunk_bytes[pos..pos + length as usize + 12])
                    .map_err(|_| PngError::InvalidBytes)?,
            );
            pos = pos + length as usize + 12;
